...
var (
	serverMetrics = metrics{
		...
		19: newServerMetric("active", "Server is active (1 = Yes, 0 = No)", nil),
		20: newServerMetric("backup", "Server is backup (1 = Yes, 0 = No)", nil),
...
func NewExporter(uri string, selectedServerMetrics map[int]*prometheus.GaugeVec, timeout time.Duration) (*Exporter, error) {
	...
	return &Exporter{
		...
		backendMetrics: map[int]*prometheus.GaugeVec{
			...
			19: newBackendMetric("active", "Number of active servers.", nil),
			20: newBackendMetric("backup", "Number of backup servers.", nil),
